...
class NEURON: 
    ...
    def Update_Hidden_Or_Motor_Neuron(self, neurons, synapses):
        ...
        for synapseKey in synapses:
            if self.Get_Name() == synapseKey[1]: # check if the current synapse arrives at the neuron being updated
                presynapticNeuronName = synapseKey[0]
                self.Allow_Presynaptic_Neuron_To_Influence_Me(synapses[synapseKey].Get_Weight(), neurons[presynapticNeuronName].Get_Value())
        self.Threshold()
    
    def Allow_Presynaptic_Neuron_To_Influence_Me(self, outgoingSynapseWeight, presynapticNeuronValue):
        self.Add_To_Value(outgoingSynapseWeight * presynapticNeuronValue)
